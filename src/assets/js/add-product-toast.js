class AddToCartToast extends HTMLElement {
  constructor() {
    ...
    this.successMessage = isArabic ? "تمت الإضافة إلى سلة التسوق" : "Added to Cart";
    this.viewCartText = isArabic ? "عرض السلة" : "View Cart";
    this.checkoutText = isArabic ? "إتمام الطلب" : "Checkout";
    this.showMoreText = isArabic ? "عرض المزيد" : "Show more";
    this.closeText = isArabic ? "إغلاق" : "Close";
    this.readyAttempts = 0;
    this.readyTimer = null;
  }

  connectedCallback() {
    if (this.isSallaReady()) {
      this.init();
    } else {
      this.readyTimer = setInterval(() => {
        ...
        if (this.isSallaReady()) {
          ...
          this.readyTimer = null;
          this.init();
        } else if (this.readyAttempts >= 100) {
          clearInterval(this.readyTimer);
          this.readyTimer = null;
        }
      }, 50);
  ...
  isSallaReady() {
    return Boolean(
      window.salla?.lang?.onLoaded &&
      window.salla?.event?.on &&
      window.salla?.cart?.api?.details &&
      window.salla?.url?.get
    );
  }

  init() {
    if (this.initialized || !this.isSallaReady()) return;
    ...
    salla.lang.onLoaded(() => {
      salla.lang.addBulk({
        ...
        "velora.toast.close": { ar: "إغلاق", en: "Close" }
      });
      ...
      this.closeText = salla.lang.get("velora.toast.close");
    });
    ...
    this.innerHTML = `
      ...
      <div class="s-add-product-toast__header">
        <div class="s-add-product-toast__header-content">
          <span class="s-add-product-toast__icon" aria-hidden="true">✓</span>
      ...
      <div class="s-add-product-toast__body">
        <a href="${this.escapeHTML(this.product.url)}" class="s-add-product-toast__image">
          <img src="${this.escapeHTML(this.product.image)}" alt="${this.escapeHTML(this.product.name)}" loading="lazy" />
        </a>
        <div class="s-add-product-toast__details">
          <a href="${this.escapeHTML(this.product.url)}" class="s-add-product-toast__name">${this.escapeHTML(this.product.name)}</a>
          ${visibleOptions.length ? `
            <div class="s-add-product-toast__options">
              ${visibleOptions.map(opt =>
                opt.hideValue
                  ? `<span>${this.escapeHTML(opt.name)}</span>`
                  : `<span>${this.escapeHTML(opt.name)}: ${this.escapeHTML(opt.value)}</span>`
              ).join("")}
...
if (!customElements.get("salla-add-product-toast")) {
